...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	walkCalories := (CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
	return walkCalories
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	swimSpead := float64(s.LengthPool) * float64(s.CountPool) / float64(MInKm) / s.Duration.Hours()
	return swimSpead
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimCalories := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * float64(SwimmingCaloriesWeightMultiplier) * s.Weight * s.Duration.Hours()
	return swimCalories
}
